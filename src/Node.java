...
public class Node implements Comparable<Node> {
  ...
  public void draw(Graphics aPen, int height, int magnification) {
    ...
    aPen.fillOval(MapperApp.MARGIN_X/2 +location.x*magnification - RADIUS, MapperApp.MARGIN_Y/2 + height - location.y*magnification - RADIUS, RADIUS * 2, RADIUS * 2);

    // Draw a black border around the circle
    aPen.setColor(Color.black);
    aPen.drawOval(MapperApp.MARGIN_X/2 +location.x*magnification - RADIUS, MapperApp.MARGIN_Y/2 + height - location.y*magnification - RADIUS, RADIUS * 2, RADIUS * 2);

    // Draw a label at the top right corner of the node
    if (label != "")
      aPen.drawString(label, MapperApp.MARGIN_X/2 + location.x*magnification + RADIUS, MapperApp.MARGIN_Y/2 + height - location.y*magnification - RADIUS);
  }
