...
public class Edge {
  ...
  // Draw the Edge at the specified magnification factor with the given Pen on a panel with the given height
  public void draw(Graphics aPen, int height, int magnification, int thickness) {
    if (selected) {
      aPen.setColor(Color.RED);
      ((Graphics2D)aPen).setStroke(new BasicStroke(thickness));
    }
    else {
      aPen.setColor(Color.black);
      ((Graphics2D)aPen).setStroke(new BasicStroke(1));
    }
    aPen.drawLine(MapperApp.MARGIN_X/2 +startNode.getLocation().x*magnification, MapperApp.MARGIN_Y/2 + height - startNode.getLocation().y*magnification,
                  MapperApp.MARGIN_X/2 +endNode.getLocation().x*magnification, MapperApp.MARGIN_Y/2 + height - endNode.getLocation().y*magnification);
    ((Graphics2D)aPen).setStroke(new BasicStroke(1));
  }
